...
export default function Navbar() {
  ...
  function handleSearchSubmit(e) {
    e.preventDefault()
    if (search.trim()) {
      navigate(`/categorias?search=${encodeURIComponent(search.trim())}`)
      setSearch('')
    ...
    <header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur-xl shadow-lg border-b border-gray-100">
      <nav className="max-w-[1600px] mx-auto flex items-center justify-between px-4 sm:px-8 h-[76px] gap-2">
        ...
        <Link to="/" className="flex items-center gap-3 font-extrabold text-2xl tracking-tight text-primary hover:opacity-90 transition-opacity select-none">
          ...
          <span className="hidden sm:inline font-black text-dark tracking-tight text-2xl" style={{letterSpacing: '-0.03em'}}>Supermercado</span>
        </Link>
